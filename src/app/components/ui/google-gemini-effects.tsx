...
export const GoogleGeminiEffect = ({
  ...
  const smallScreenSize = useMediaQuery("(min-width:960px)");
  const buttonHeight = smallScreenSize ? "h-[890px]" : "h-[800px]";
  const strokeWidth = smallScreenSize ? "3" : "8";
  const parallaxTopHeight = smallScreenSize ? "top-80" : "top-[10rem]";
  const svgWidth = smallScreenSize ? "3000" : "1440";
  const svgHeight = smallScreenSize ? "2200" : "890";
  const svgViewBoxString = smallScreenSize
    ? "0 300 1440 890"
    : "0 -500 1440 890";

  return (
    <div className={cn(`sticky ${parallaxTopHeight}`, className)}>
      <p className="text-lg md:text-7xl font-normal pb-4 text-center bg-clip-text text-white text-transparent bg-gradient-to-b from-neutral-100 to-neutral-300">
        A Visual Prototyping
        <span className="text-img-main"> Tool For React </span>
        Developers
      </p>
      <p className="lg:text-[2rem] md:text-[2rem] sm:text-xl xs:text-xl font-normal text-center text-neutral-400 mt-4 mx-auto">
        Built-in comprehensive type checking with TypeScript and flexible
        exporting.
      </p>
      <div
        className={`w-full ${buttonHeight} -top-60 md:-top-40  flex items-center justify-center bg-red-transparent absolute`}
      >
        <Button
      ...
      <svg
        width={svgWidth} // Adjust width as needed
        height={svgHeight} // Adjust height as needed
        viewBox={svgViewBoxString}
